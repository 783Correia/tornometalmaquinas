...
export default function AdminCategorias() {
  ...
  async function handleSave() {
    ...
    if (editing) {
      await supabase.from("categories").update({ name: form.name, slug }).eq("id", editing);
    } else {
      await supabase.from("categories").insert({ name: form.name, slug });
    }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Categorias ({items.length})</h1>
